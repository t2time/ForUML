...
public class FortranStream extends ANTLRFileStream {
  ...
  /**
   * Create a new input buffer and use it to fix line continuation. This buffer
   * will be used to strip out continuation characters, extra '\n' characters,
   * and in fixed form, extra characters in columns 1-6, including TAB. It also
   * moves comments and preprocesser commands if they are in the middle of
   * continuation lines.
   *
   * Note that Holleriths in edit descriptors must be recognized, otherwise what
   * looks like a comment will be processed incorrectly, consider the the very
   * evil
   *
   ...
   * building tokens, in particular key words, the actually character buffer is
   * unchanged so id tokens have original case.
   */
  public int LA(int i) {
    ...
    /**
     * **OBSOLETE***** int char_pos = super.getCharPositionInLine();
     *
     * if (this.sourceForm == FrontEnd.FIXED_FORM) {
     * System.out.println("FIXED_FORM: i=" + i + " pos=" + char_pos + " " +
     * letter_value);
     *
     * if (char_pos == 0) { if (letter_value == 'C' || letter_value == '*') { //
     * return '!' to signify a line comment so the lexer won't try // and parse
     * this line. letter_value = (int) '!'; } } else { // Look for continuation
     * character. The convention we use // is for TAB + (WS | '0') acts as ';'.
     * TAB + other char is a // continuation line. This follows DEC (I believe)
     * but is // non standard Fortran. //
     *
     * // location of continuation character int continue_pos = 5; // 6th column
     *
     * // check for tab formatting, note, this seems to work because LA always
     * // called with i==1, so getCharPositionInLine is always as expected if
     * (super.LA(-char_pos) == (int) '\t') { // first character in line is a tab
     * continue_pos = 1; }
     *
     * if (char_pos == continue_pos) { // if neither '0' nor whitespace then a
     * continuation character if (! (letter_value == (int)'0' ||
     * Character.isWhitespace((char)letter_value))) { letter_value = (int)'&'; }
     * } } } return letter_value;
*****OBSOLETE****
     */
  } // end LA()
   ...
   * excluding the terminating '\n' character. Because the next line could be a
   * fixed form continuation, we can't immediately consume the comment as all
   * comments must come after all continued lines (comments can be interspersed
  ...
  /**
   * Check for comment characters, 'C', '*', and '!' at start of a line. A blank
   * line is also a comment line. If a comment line is found, copy the line
   * comment to the comments buffer (without trailing '\n', and return the
   * position of the character after the '\n' character.
   */
   ...
   * column 6. There could be comment or preprocess lines in between so have to
   * skip over comment lines and if they exist.
   *
   * The convention for a TAB character in columns 1..5 followed by a digit 1..9
   * is a continuation line. If TAB + '0' the '0' is ignored in the input
   ...
   * Called when at a '\n' character. Look ahead for continuation character at
   * column 6. There could be comment or preprocess lines in between so have to
   * search for comment lines and remove them if they exist.
   *
   * The convention for a TAB character in columns 1..5 followed by a digit 1..9
   * is a continuation line. If TAB + '0' the '0' is ignored in the input
   * stream. This follows DEC convention (I believe) but is non standard
   ...
   * followed by 'H'|'h' and then n characters (none of which is \'n'). Perhaps
   * we want to change Hollerith to a quoted string. In any case, copy string
   * representation to newBuf.
   *
   ...
   *
   * Assume that strings have been matched so a Hollerith-like constant within a
   * string doesn't have to been considered.
   *
   ...
   * '&' character on the next line; return the position of the trailing '&'. If
   * a string doesn't terminate it's an error, return '\n' position.
   */
