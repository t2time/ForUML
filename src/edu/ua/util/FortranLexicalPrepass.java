...
public class FortranLexicalPrepass {
   ...
   * (tokens.getToken(start).getType() == FortranLexer.T_ASSIGNMENT && (start+3
   * < end) && tokens.getToken(start+1).getType() == FortranLexer.T_LPAREN &&
   * tokens.getToken(start+2).getType() == FortranLexer.T_EQUALS) { return true;
   * } else { return false; } } // end isAssignment() END OBSOLETE*******
   */
  /**
   ...
   * tokens.getToken(start+3).getType() == FortranLexer.T_RPAREN) { return true;
   * }	else { return false; } } // end isOperator() END OBSOLETE*******
   */
  /**
   * Convert keyword tokens (except for generic-spec items) from start to end to
   * identifiers. Tokens to convert often represent expressions which can be
   * primaries which can be array constructors. So must look for occurrence of
   * an array constructor of form [ type_spec_stuff :: ... ] and don't convert
   * the intrisic-type-spec keyword.
   */
   ...
   * constants. Actually, the lexer already sucks up quotes (single and double)
   * into the T_CHAR_CONSTANT tokens that it creates, so no need to consider
   * here.
   */
  ...
  /**
   * Returns true if the token at i is a name. Since at this stage a keyword can
   * also be a name, a name is (id | keyword).
   */
   ...
   *
   * A type-spec is a name|keyword followed by optional parens. Note: but could
   * mistakenly match a function call so have to be careful with interpreting
   * results.
   */
  ...
  /**
   * Match the various types of end statments. For example: END, ENDSUBROUTINE,
   * ENDDO, etc.
   */
  ...
  private boolean matchUseStmt(int lineStart, int lineEnd) {
    ...
    if (lexer.isKeyword(tokens.currLineLA(identPos + 1))) // the module name is a keyword so convert it.
    {
    ...
    if (identPos < lineEnd) {
      ...
      if (tokens.currLineLA(identPos + 1) == FortranLexer.T_COMMA
              && tokens.currLineLA(identPos + 2) == FortranLexer.T_ONLY) // Skip the T_COMMA, T_ONLY, and T_COLON.
      {
        identPos += 3;
      }
   ...
   * assumes that the T_EOS tokens in a multi-line statement are removed for all
   ...
   * return true if they are found. The T_CHAR_CONSTANT token following
  ...
  private boolean matchProcStmt(int lineStart, int lineEnd) {
    ...
    if (tokens.currLineLA(lineStart + 1) == FortranLexer.T_PROCEDURE
      ...
      if (lineEnd < (lineStart + 4)) // it is a module stmt
      {
        return false;
      }
      identOffset = lineStart + 2;
   ...
   * procedure statements, and MUST be called AFTER trying to match a procedure
   * statement.
   */
  ...
  /**
   * Return the offset of the beginning of an array constructor, ie, the offset
   * of '[' or the offset of the '/' in the '(' '/' sequence, otherwise return
   * -1.
   */
  ...
  /**
   * The token at lineStart initiates an intrinsic or derived type-spec; convert
   * kind/len expressions to identifiers.
   */
...
// TODO - fix for T_BIND token
  ...
  /**
   * Find the first index after the typespec (with or without the optional kind
  ...
  private int skipTypeSpec(int lineStart) {
    ...
    if (isIntrinsicType(firstToken) == true
      ...
      if (rparenOffset != -1) // rparenOffset will be the logical index of the right paren.
  ...
  private int matchVList(String line, int lineIndex) {
    ...
    if (line.charAt(tmpLineIndex) == '(') {
      ...
      if (tmpLineIndex == lineLength) /* There is an error in the line!  */ {
        return lineIndex;
      }
  ...
  private boolean isValidControlEditDesc(String line, int lineIndex) {
    ...
    if (firstChar == ':' || firstChar == '/' || firstChar == 'p'
      ...
      if (firstChar == 's') /* TODO: verify the following is true for sign-edit-desc.  */ /* If the first char is an 'S', then it can be followed by another 
...
//      } while(nextChar != '\'' && nextChar != '"');
   ...
   * format-specification are just converted by the lexer to identifiers so now
   * the edit descriptors need to be parsed, primarily by going back to the
   * original character representation of the input stream.
   */
  ...
  /**
   * Parse the format-specification in a format for edit descriptors. Although a
   * Hollerith edit descriptor has been deprecated, deal with it anyway. All
   ...
   * fail Sale's because they will have an equal sign and no comma. Sale's says
   * that it must not start w/ a keyword then, but these are exceptions. They
   * could also have no equal and no comma, such as: if (result < 0.) cycle
   */
  ...
  private boolean matchGenericBinding(int lineStart, int lineEnd) {
    if (tokens.currLineLA(lineStart + 1) == FortranLexer.T_GENERIC) {
      ...
      if (nextToken == FortranLexer.T_OPERATOR
        ...
        int rparenOffset;
        if (tokens.currLineLA(nextTokenLA + 1) != FortranLexer.T_LPAREN) // syntax error in the spec.  parser will report
        {
          return false;
        }
...
// TODO - fix to T_BIND token
   ...
   * Token token; int i = 0;
   *
   * if(startsWithKeyword == true) { do { System.out.println("fixed-format line
   * must start with keyword"); tokens.printPackedList(); buffer =
   ...
   * continuation"); token = myLexer.nextToken(); // System.out.println("lexer
   * said next token.getText() is: " + // token.getText()); //
  ...
  public void performPrepass() {
     ...
     * entering this method in antlr-3.0b*, but in antlr-3.0, the number // was
     * no longer guaranteed to be -1. so, seek the index ptr to -1. if
     * (tokensStart != -1) { tokens.seek(-1); tokensStart = -1; } END
     * OBSOLETE*****
     */
    while (tokens.LA(1) != FortranLexer.EOF) {
      ...
      if (commaIndex != -1) {
        ...
        if (equalsIndex != -1) {
          /**
           * ******OBSOLETE // TODO: // have to figure out how to rearrange the
           * case where we // can't start with a keyword (given the tests below
           * fail) for // fixed format where we may have to combine tokens to
           * get the // statement to be accepted. // if(this.sourceForm ==
           * FrontEnd.FIXED_FORM) { // fixupFixedFormatLine(lineStart,
           * lineLength, false); // } END OBSOLETE*****
           */
          ...
          /**
           * ******OBSOLETE // TODO: // need to make sure that this can be here
           * because it may // prevent something from matching below... //
           * if(this.sourceForm == FrontEnd.FIXED_FORM) { //
           * fixupFixedFormatLine(lineStart, lineLength, true); // } END
           * OBSOLETE******
           */
